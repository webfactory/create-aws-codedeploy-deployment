...
This makes it possible to create a matching configuration once, and then run deployments in different environments automatically. For example, updating a production system for commits or merges on `main`, and independent staging environments for every open Pull Request branch.
...
Due to the first few lines in this example, the action will be run for commits pushed to the `main` branch and for Pull Requests
...
4. Add the `branch_config` section to your `appspec.yml` file to map branches to Deployment Groups and their configuration. In the above example, the  `main` and `.*` sub-sections show the minimal configuration required.
